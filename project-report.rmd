...
Before noticing a major bug in the code, the PLS outcomes were dominated by variables relating to the synergies between apple and chicken subsystems. In the debugged version, however, none of the variables appear to matter in the PLS or EVPI analysis. But this does not necessarily mean, that the synergies are neglectable. For once, the variables were mostly approximated by our group memebers, because the data coverage is low and the results are ambiguous. Consulting experts on the chosen ranges and even more importantly on the implementation of the synergies (and trade-offs) might be necessary. 

possible further topics:
- the amount and costs of field operations are unrealistic. For example the amount of time and money spent on mowing appears to be overestimated but the spraying to be underestimated in the input table when comparing to a 'typical' year in the KTBL spreadsheet tool. 
- implementation of synergies is maybe too optimistic, gradual decrease, less incidence = less control cost? say that relevant even if not in PLS / EVPI because synergies push positive results of pure chicken to apple chicken from 59% to 61%, so in 200 of the 100,000 cases it decided whether the decision is beneficial or not. 
- another aspect which cannot be covered by the decision analysis is, that in reality the farmer doesn't compare wether an investment makes him better off, but compares several options for investment against each other. It appears that the additionally gained median NPV is rather low compared to the strong investments and maybe more importantly changes in the management. Maybe the farmer prefers another investment (lets say another variety, irrigation infrastructure) which strengtehn their main business inestead of diversifying their operations. 
